...
Per poter utilizzare efficacemente i plugin di PhoneGap è necessario predisporre il codice JavaScript al loro utilizzo. In particolare sono necessari due accorgimenti che vengono di seguito presentati.
...
Il plugin \textit{dialogs} fornisce accesso all'interfaccia grafica nativa degli elementi dialog tramite l'utilizzo dell'oggetto \textit{navigator.notification}. Questo plugin è stato utilizzato per convertire gli alert box delle pagine web in dialog nativi. Sono stati utilizzati i seguenti metodi:
\begin{itemize}
...
\begin{itemize}
	...
	\item \textit{online}: viene lanciato quando un dispositivo precedentemente scollegato dalla rete riceve la connessione e permettendo all'applicazione di accedere ad Internet. In particolare, viene lanciato esattamente quando il valore della proprietà \textit{type} cambia da \textit{NONE} ad un altro valore.
\end{itemize}
...
Vengono di seguito forniti, a scopo illustrativo, degli esempi di codice JavaScript che utilizzano \textit{network.connection} ed in particolare la proprietà \textit{type} e l'evento \textit{offline}.
