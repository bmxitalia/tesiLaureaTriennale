...
\begin{itemize}
	...
	\item \textbf{Output}: le possibili risposte del \textit{servlet} sono le seguenti:
		...
		\end{itemize}
		\item \textbf{Descrizione}: questo \textit{servlet} rappresenta il servizio di autenticazione. Ricevuti i parametri, effettua la ricerca della \textit{username} nel \textit{database} e, nel caso in cui questa fosse presente, procede nel cercare la \textit{password} corrispondente. Se \textit{username} e \textit{password} passati sono corretti, il \textit{servlet} restituisce una stringa in formato \textit{JSON} contenente il codice azienda e la \textit{username} dell'utente che ha cercato di effettuare il \textit{login}. Se le credenziali non sono corrette oppure l'utente è stato bloccato, viene restituito un messaggio d'errore.
\end{itemize}
